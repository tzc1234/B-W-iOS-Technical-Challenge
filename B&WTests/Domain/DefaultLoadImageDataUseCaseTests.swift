...
final class DefaultLoadImageDataUseCaseTests: XCTestCase {
    ...
    private func makeSUT(file: StaticString = #filePath,
                         line: UInt = #line) -> (sut: DefaultLoadImageDataUseCase, repository: ImageDataRepositorySpy) {
        let repository = ImageDataRepositorySpy()
        let sut = DefaultLoadImageDataUseCase(repository: repository)
        trackForMemoryLeaks(repository, file: file, line: line)
        trackForMemoryLeaks(sut, file: file, line: line)
        return (sut, repository)
    }
    
    private func expect(_ sut: DefaultLoadImageDataUseCase,
